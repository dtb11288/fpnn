...
module.exports = config => {
  const createBrain = layers => {
    ...
    const train = (inputs, targets) => pipe(
      arrayToOneColMatrix,
      calculateOutputs,
      allOutputs => {
        ...
        return mapIndexed((layer, index) => layer.adjustLayer(allOutputs[index], allOutputs[index + 1], allErrors[index], config.learning_rate), layers)
      },
      createBrain
    )(inputs)

    const toJSON = () => ({ layers: map(l => l.toJSON(), layers) })
  ...
  const initBrain = nodes => pipe(
    defaultTo([]),
    when(compose(lt(__, 2), length), () => { throw new Error('require input length at least 2') }),
    aperture(2),
    map(([input, output]) => L.initLayer(input, output)),
    createBrain
  )(nodes)

  const trainMultiple = curry((data, brain) => reduce((brain, { inputs, targets }) => brain.train(inputs, targets), brain, data))
